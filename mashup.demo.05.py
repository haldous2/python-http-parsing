...
def extract_restaurant_metadata(elem):
    ...
    ## td with id equal to 'contentcol' ==> div with id like 'PR*~'' ==> elem

    r_data_trs = elem.find('tr')
    #print r_data_trs

    if r_data_trs.name == 'tr':
        r_data_tds = r_data_trs.findAll( has_two_tds, recursive=False )
    else:
        r_data_tds = []
    ...
    rdata = {}
    current_label = ''
    for data_row in r_data_tds:
        key_cell, val_cell = data_row.findAll('td', recursive=False)
...
def get_score_data(elem):
    inspection_rows = elem.findAll(is_inspection_data_row)
    samples = len(inspection_rows)
    total = 0
    high_score = 0
    average = 0
    for row in inspection_rows:
        strval = clean_data(row.findAll('td')[2])
        try:
            intval = int(strval)
...
if __name__ == '__main__':
    ...
    for result in result_generator(1):
